...
$api = $_GET["api"];
$tabela = $_GET["tabela"] ? $_GET["tabela"] : "empresa";
$limite = $_GET["limite"] ? $_GET["limite"] : "150";
...
if ($api) {
   ...
   $sth = 
	...
	JOIN servico ON atendimento.id_servico = servico.id_servico
...
$sth = $dbh->prepare("SELECT * FROM documento limit 1");
   ...
   //$sth = $dbh->prepare("SELECT * FROM cliente limit 10");
   //$sth = $dbh->prepare("SELECT * FROM ".$tabela." limit $limite");
   $sth->execute();
