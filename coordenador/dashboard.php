            ...
            <div class="col-12">
               ...
               <div class="mb-2">
                  ...
                  <div class="collapse d-md-block" id="displayOptions">
                     ...
                     <select id="inputConsultor" name="inputConsultor">
                        <option value="0" selected="">Consultor</option>
                     </select>
                     <select id="inputMidia" name="inputMidia">
                        <option value="0" selected="">Mídia</option>
                     </select>
               ...
               <div class="separator mb-5 mt-2"></div>
            </div>
         </div>
   ...
   <script>
      ...
      function criarLinhaLista(item) {
         var linha = '<div class="card d-flex flex-row mb-3">' +
            ...
            '<p class="mb-1 text-muted text-small w-15 w-sm-100"><i class="simple-icon-clock"></i> ' + formatarData(item.dt_inclusao) + ' ' + formatarHora(item.dt_inclusao) + '</p>' +
      ...
      function criarLinhaBloco(item) {
         var linha = '<div class="card d-flex flex-row mb-4">' +
            ...
            '<p class="list-item-heading mb-1 truncate">' + item.cliente.nome_cliente + '</p>' +
            '</a>' +
            '<p class="mb-2 text-muted text-small">' + item.consultor.nome + '</p>' +
      ...
      function recarregarHotLead() {
         if (sessionStorage.getItem("visaoBlocos")) {
            $.ajax({
               url: '../utils/listarAtendimentosDia.php',
               type: 'POST',
               data: {
                  idConsultor: $("#inputConsultor").val(),
                  idMidia: $("#inputMidia").val()
               },
                  ...
                  if (data != false) {
                     ...
                     $.each(jsonData, function(key, value) {
                        switch (value.fluxo_atendimento.nome) {
                           case "HOT LEAD":
                              var linha = criarLinhaBloco(value);
      ...
      function carregarPainelLead() {
         if (sessionStorage.getItem("visaoBlocos")) {
            ...
            $.ajax({
               ...
               complete: function(data) {
                  $("#app-container").removeClass("show-spinner");
                  //setInterval(() => recarregarHotLead(), 100000);
               }
            });
         }
      }
         ...
         switch (getUrlParameter('falha')) {
            case 'true':
               if (getUrlParameter('tipo') == "update") {
                  ...
                  $.notify({
                     // options
                     message: 'Erro ao atualizar o <strong>Seu Perfil!</strong>'
                  }, {
                     // settings
                     type: 'danger'
                  });
